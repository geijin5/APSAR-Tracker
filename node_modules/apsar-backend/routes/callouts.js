...
router.post('/', auth, authorize('admin', 'officer'), upload.array('attachments', 10), async (req, res) => {
  try {
    const calloutData = {
      ...req.body,
...
// @desc    Update callout
// @access  Private - Admin/Operator
router.put('/:id', auth, authorize('admin', 'officer'), upload.array('attachments', 10), async (req, res) => {
  try {
    const callout = await Callout.findById(req.params.id);
...
// @access  Private - Admin/Operator
router.post('/:id/attach-report/:reportId', auth, authorize('admin', 'officer'), async (req, res) => {
  try {
    const callout = await Callout.findById(req.params.id);
